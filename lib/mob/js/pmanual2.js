...
function getMenu() {
	...
	s+="<span id='pmerge' style='"+cs2+"'><span style='color:red'>[F2]</span> accept/merge block</span><br>";
	s+="<span id='pgoparent'><span style='color:blue'>[F3]</span> select parent block</span><br>";
	s+="<span id='pdelete'><span style='color:blue'>[F4]</span> delete block</span><br>";
	s+="<span id='pflatten'><span style='color:#EF2BB5;'>[F7]</span> flatten</span><br>";
	s+="<span id='pdetailed' style='"+cs8+"'><span style='color:green;'>[F8]</span> fine block selection</span><br>";
	s+="<span id='pinsert' style='"+cs9+"'><span style='color:green;'>[F9]</span> insert block on element</span><br>";
	s+="<span id='pinscustom' style='"+csc+"'><span style='color:green;'>[Ctrl-F9]</span> insert custom block</span><br>";
	s+="<span id='pshowhide'><span style='color:green'>[F10]</span> hide/show panel</span><br>";
	s+="<span id='psend' style='"+cs12+"'><span style='color:#008000;'>[F12]</span> send to server</span><br>";
	s+="<span id='pcancel'><span style='color:orange'>[ESC]</span> cancel/deselect</span><br>";
	return(s);
}
...
function carga(e) {
		...
		if (!cargado) {
			...
			if (!pac) pac=5;
			//splash = window.open(chrome.extension.getURL("html/splash.html"),"_blank","left=100,top=100,width=400,height=400")
			//~ chrome.windows.create(chrome.extension.getURL("html/splash.html"),"detached_panel");
			sourceurl = location.href;
			startSegmentation(window,pac,50,"record",sourceurl);
			//~ chrome.windows.remove(splash);
...
function dale(e) {
	...
	if ( !editing && 
		...
		if (!lastLog) return;
		document.getElementById("mobstatus").innerHTML = "<b>"+lastLog.id+"</b><br><b>Granularity:</b>: "+lastLog.gran.toFixed(4) + "<br><b>HTMLCover:</b> "+lastLog.htmlcover+"<br><b>WordCover:</b> "+lastLog.wordcover+" ("+(lastLog.wordcover/page.wordcover).toFixed(2)+"%)";
