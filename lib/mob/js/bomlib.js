...
var verbose = false; //for debugging. Write DIVS representing blocks  in live web page.
 ...
 */
function startSegmentation(win, pac, pdc, returnType, pverbose) {
    contentWindow = win;
...
function isTextElement(element) {
    return !!(element && element.nodeName == "#text")
}
...
function Grid(rows, cols) {
    ...
    this.getImportance = function(block) {
        ...
        var acum = 0;
        for (var i = initialYpos; i <= finalYpos && i < this.rows; i++) {
            for (var j = initialXpos; j <= finalXpos && j < this.cols; j++) {
                acum += this.get(i, j);
            }
        }
        return(acum);
        ...
        for (var j = 0; j < log.geometricObjects.length; j++) {
            ...
            for (var k = 0; k < all.length; k++) {
                ...
                if (!child.hasAttribute("css_added")) {
                    child.setAttribute("style", getLCSS(child));
                }
